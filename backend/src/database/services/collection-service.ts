...
export class CollectionService {
    ...
    public async getUserRoleInCollection(
        ...
        type OrganisationWithRole = (typeof organisations)[number];

        const collectionRoles = await Promise.all(
            organisations.map(
                async (organisation) =>
                    [
                        organisation,
                        await this.collectionRepository.getOrganisationRoleInCollection(
                            collectionEntityId,
                            organisation.id
                        ),
                    ] satisfies [
        ...
        // return the highest role the user has in the collection,
        //
        // important if they are part of multiple organisations
        ...
        switch (highestOrganisationRole?.[1]) {
            case 'owner':
                return highestOrganisationRole[0].userRole;
            case 'viewer':
                return 'viewer';
            case 'other':
                return 'viewer';
    ...
    public async removeCollectionOrganisation(
        ...
        return this.collectionRepository.removeCollectionOrganisation(
            collectionEntityId,
            organisationId
        );
    }
    ...
    public async joinCollectionByCode(
        joinCode: string,
        organisationId: OrganisationId
    ) {
        return this.collectionRepository.transaction(async (tx) => {
    ...
    public async getLatestUsableCollections(
        ...
        const userCollections = await this.getLatestCollectionsForUser(user, {
    ...
    public async archiveCollection(
        ...
        const result = this.exists(
            await this.collectionRepository.archiveCollection(
                collectionEntityId,
                unarchive
            )
        );
    ...
    public async deleteElementFromCollection(
        ...
        return this.reduce(
            ...
            async (tx, draftState, eventBuffer) => {
                ...
                if (containingSet.entityId !== collection.entityId) {
                    throw new Error(
