...
Eine neue Übung kann auf der Startseite durch Klicken auf den Button "Übung erstellen" erstellt werden. Dazu sind weder ein Benutzerkonto noch eine anderweitige besondere Berechtigung notwendig. Nach dem erfolgreichen Erstellen werden die Übungsleitungs- und die Teilnehmenden-PIN unterhalb des Buttons angezeigt. Die Übungsleitungs-PIN wird zudem automatisch in das Formularfeld "Übungs-PIN" im linken Teil der Seite eingetragen, sodass ein [Beitritt zur neu erstellten Übung](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungssbeitritt) als Übungsleitung mit nur einem Klick möglich ist. 

Für Nutzende mit einem Benutzerkonto ist es zudem möglich, Übungsvorlagen zu erstellen und zu bearbeiten. Aus Vorlagen lassen sich dann durchführbare Übungen erstellen. Näheres dazu in [4 Übungselemente und -vorlagen](../4%20Elemente%20und%20Vorlagen.md).

## Einer Übung beitreten

Um einer Übung über die Startseite beizutreten, muss die entsprechende [Übungs-PIN](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungs-pins) in das Formularfeld "Übungs-PIN" im linken Teil der Seite eingegeben und auf "Übung beitreten" geklickt werden. 
...
Für das Beitreten zu einer Übung gibt es verschiedene Arten von PINs für unterschiedliche Berechtigungen bzw. Teilnahmearten. Ein Beitritt ist sowohl über die Eingabe der PIN [auf der Startseite](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungssbeitritt) als auch direkt über das Öffnen der [teilbaren Links](https://outline.jonathanweth.de/doc/22-ansichten-Say3tcwyfv#h-einladungs-buttons) (Format: `fuesim-manv.de/exercises/<PIN>`) möglich.

### Teilnehmenden-PIN

Die Teilnehmenden-PIN ist sechsstellig und ermöglicht den Beitritt als Teilnehmender. Teilnehmende müssen zur aktiven Mitwirkung in einer Übung durch die Übungsleitung freigegeben werden und können auf eine bestimmte [Ansicht](https://outline.jonathanweth.de/doc/22-editorubungselemente-Vui2CqCp9T#h-ansichten) eingeschränkt werden.
...
Um die jeweiligen PINs einfach an weitere Nutzende weiterzugeben und diese zur Übung hinzuzufügen, gibt es in einer Übung in der oberen Menüleiste die Buttons "Teilnehmende einladen" und "Übungsleitende einladen". Näheres dazu unter [22 Übungsansicht](2_uis.md).
...
Für die Übungsleitung sind hingegen sämtliche Interaktionen mit Patienten, Fahrzeugen und Personal bereits möglich. Allerdings beginnen die Sichtung und Behandlung bzw. Verschlechterung der Patienten noch nicht, selbst wenn das Rettungspersonal korrekt platziert wird. Auch gestartete Transfers laufen noch nicht. Mit einem Klick auf "Sofort" in der [Transferübersicht](https://outline.jonathanweth.de./24%20Durchf%C3%BChrung.md#h-transferubersicht) können Fahrzeuge und Personal allerdings aus dem Transfer herausgenommen und wieder auf der Karte platziert werden. 

Übungsleitende können im Zustand „Vorbereitung" bereits Teilnehmende zu einer Übung hinzufügen und diese in Vorbereitung auf den Übungsstart in bestimmte Ansichten einteilen (siehe [Teilnehmende verwalten](https://outline.jonathanweth.de./24%20Durchf%C3%BChrung.md#h-teilnehmende-verwalten)).
...
> [!TIP]
> Es wird unbedingt empfohlen, die Übung auch am Ende zu pausieren, damit bei der [25 Auswertung](5_evaluation.md) nur die relevanten Zeiträume in Statistik und Zeitstrahl betrachtet werden.
...
> [!WARNING] 
> Für eines der nächsten Updates ist geplant, dass nicht genutzte Übungen von Nutzenden ohne Konto nach einer gewissen Zeit ohne Verwendung automatisch gelöscht werden. Es wird deshalb empfohlen, relevante Übungsinhalte zu [exportieren](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungsinhalte-exportieren) oder ein [Benutzerkonto anzulegen](https://outline.jonathanweth.de./../5%20Benutzerverwaltung.md#h-registrierung), in dem die Übung verwaltet wird. 

## Übungsinhalte exportieren

Durch Klick auf den Button "Exportieren" in der oberen Menüleiste innerhalb einer Übung kann diese ganz oder teilweise exportiert werden. Standardmäßig wird als Export eine [`.json`-Datei](https://de.wikipedia.org/wiki/JSON) mit der Übungs-PIN im Dateinamen zum Download bereitgestellt.

### Übungen exportieren

Die Optionen "kompletter Verlauf" und "aktueller Zustand" ermöglichen jeweils den Export der jeweiligen Übung, die dann über die Startseite zur [Erstellung einer neuen Übung](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungen-importieren) importiert werden können. Alternativ kann ein Übungsexport auch als [neue Übungsvorlage in das eigene Benutzerkonto](https://outline.jonathanweth.de./../5%20Benutzerverwaltung.md#h-ubungsvorlagen-verwalten) importiert werden.
...
Der "komplette Verlauf" enthält dagegen zusätzlich ein vollständiges Protokoll aller bisherigen Aktionen durch Übungsleitende und Teilnehmende. Ein Export des Verlaufs ist dann sinnvoll, wenn eine abgeschlossene Übung für eine spätere [25 Auswertung](5_evaluation.md) gesichert werden soll. 

> \[!NOTE\]
>
> Der Verlauf einer Übung umfasst auch ein Protokoll jedes einzelnen (Zwischen-)Schrittes während der Übungserstellung durch die Übungsleitung (z.B. das Platzieren, Verschieben, Löschen, Benennen und Umbenennen von Elementen). Da alle diese Schritte zum Zeitpunkt 0:00 passieren, kann dieses Erstellungsprotokoll nicht als [Aufzeichnung](https://outline.jonathanweth.de./25%20Auswertung.md#h-aufzeichnung) in der FüSim Digital abgespielt werden, es kann aber von technisch versierten Personen aus der `.json`-Datei ausgelesen werden.

### ~~Vorlagen exportieren~~ (aktuell deaktiviert)

~~Mit der Option "Vorlagen exportieren" ist es möglich, die bereits erstellten Vorlagen für Patienten, Fahrzeuge und Bilder (siehe~~ [23 Editor/Übungselemente](3_exercise_elements.md)~~) zu exportieren, um diese für andere Übungen wiederzuverwenden (siehe~~ [~~Vorlagen importieren~~](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-vorlagen-importieren)~~). Es kann ausgewählt werden, welche dieser Vorlagentypen Teil des (partiellen) Exports sein sollen.~~

> \[!WARNING\] \nDer **partielle Export** von Vorlagen steht aufgrund von Umbauarbeiten für eine neue Version der Software, welche umfangreiche Funktionen zum Verwalten von Vorlagen enthalten wird, **nicht mehr zur Verfügung**. Als Alternative besteht die Möglichkeit, eine [vollen Übungsexport](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungen-exportieren) durchzuführen und in der `.json`-Datei nicht benötigte Elemente zu löschen. Der Import von alten partiellen Exporten ist weiterhin unter [Vorlagen importieren](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-vorlagen-importieren) möglich.
...
Auf der Startseite gibt es (neben dem Button zum [Anlegen einer neuen Übung](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungen-anlegen)) die Möglichkeit, eine Übung zu importieren. Wird dort eine passende (d. h. aus der FüSim Digital [exportierte](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungen-exportieren)) `.json`-Datei hochgeladen, wird eine neue Übung angelegt und mit einer Kopie der Daten aus der Datei gefüllt.

> \[!NOTE\]\nDie neu angelegte Übung hat nach dem Import komplett neue [Übungs-PINs](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-ubungs-pins) und keinen Zusammenhang mehr zur alten Übung, deren Daten exportiert wurden, und deren PINs.

### Vorlagen importieren

Um bereits erstellte Patienten, Fahrzeuge und Bilder (siehe [23 Editor/Übungselemente](3_exercise_elements.md)) wiederzuverwenden, gibt es am oberen Rand des [Editors](https://outline.jonathanweth.de/doc/22-ubungsansicht-Say3tcwyfv#h-editor) den Button "Vorlagen importieren". 
...
Übungsleitende können in der [unteren Menüleiste](https://outline.jonathanweth.de/doc/22-ansichten-Say3tcwyfv#h-untere-menuleiste-nur-in-ubungsleitungs-ansicht) den Punkt "Einstellungen" wählen. Es öffnet sich ein Fenster, in dem globale Einstellungen für die aktuelle Übung vorgenommen werden können.
...
In diesem Bereich kann die Karte für die primäre [Übungsansicht](https://outline.jonathanweth.de/doc/22-ansichten-Say3tcwyfv#h-ubungsansicht) konfiguriert werden.
...
In diesem Bereich kann die Karte für die [Einsatzansicht](https://outline.jonathanweth.de/doc/22-ansichten-Say3tcwyfv#h-einsatzansicht-fur-teilnehmende) konfiguriert werden.

Die wichtigste Einstellung ist die **Serveradresse**, die analog zur oben näher beschriebenen [Einstellung für die Übungskarte](https://outline.jonathanweth.de/doc/21-allgemeines-aHCOZA4Gjg#h-karte) funktioniert.

Zusätzlich kann eine **Serveradresse für Gebäudedaten** angegeben werden. Hierbei ist ein Kartendienst erforderlich, welcher Vektorkarten bereitstellt, welche eine sogenannte Gebäude(`building`)-Ebene enthalten.
...
In diesem Bereich werden die patientenbezogenen Aspekte der Übung konfiguriert (siehe [Patienten](https://outline.jonathanweth.de./23%20%C3%9Cbungselemente.md#h-patienten)).

Mit der Einstellung "**Mit Vorsichtung üben?**" wird das Vorsichten von Patienten aktiviert oder deaktiviert. Standardmäßig ist die Vorsichtung aktiviert. Wenn die Vorsichtung deaktiviert ist, haben alle Patienten bereits zu Übungsbeginn eine Sichtungsfarbe. Außerdem ist eine manuelle Anpassung der Sichtungskategorie durch Teilnehmende im Patienten-Popup nicht mehr möglich.

Die Einstellung "**Mit SK IV Patienten üben?**" regelt, ob SK IV (blau) verfügbar ist. Wenn sie deaktiviert ist, wird die Sichtungsfarbe SK IV (blau) dort, wo sie gesetzt ist, durch SK I (rot) ersetzt.

Im Feld "**Präfix der Patienten-IDs**" kann ein Präfix eingegeben werden, das allen Patienten-IDs in der Übung vorangestellt wird. Mit dieser Einstellung lassen sich Patienten-IDs an ein ortsübliches Schema anpassen, z.B. indem das Ortskennzeichen (z.B. `B-`) vorangestellt wird. Die eigentlichen IDs (der Teil hinter dem Präfix) sind eine bei `0001` beginnende Zahl, die mit vier Ziffern dargestellt wird.
...
In diesem Bereich kann eine optionale Statusanzeige für [Fahrzeuge](https://outline.jonathanweth.de./23%20%C3%9Cbungselemente.md#h-fahrzeuge) aktiviert werden.
